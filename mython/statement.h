...
// Отдельный класс исключений для перехвата и хранения 
// результата исполнения инструкции Return
class ReturnException : public std::exception
{
public:
    explicit ReturnException(const runtime::ObjectHolder& rex_value);
    // Возвращает ObjectHolder, хранящий переданный результат инструкции Return
...
/*
Вычисляет (возвращает) значение переменной либо цепочки вызовов полей объектов id1.id2.id3.
...
{
public:
    explicit VariableValue(const std::string& var_name);            // Конструктор для 1 переменной
    explicit VariableValue(std::vector<std::string> dotted_ids);    // Конструктор для цепочки переменных

    // Возвращает значение единственной переменной или значение последней переменной в цепочке
...
{
public:
    // Конструктор экземпляра класса class_ без аргументов
...
private:
    //const runtime::Class& cls_;  // не будем хранить Class, сразу создадим Instance
...
public:
    ...
    {}
protected:
    std::unique_ptr<Statement> argument_;  // protected, чтобы иметь доступ в дочерних классах
};

// Операция str, возвращающая строковое значение своего аргумента, 
// унаследованного от родительского класса UnaryOperation
class Stringify : public UnaryOperation
{
public:
    using UnaryOperation::UnaryOperation;
    // Execute() для Stringify возвращает ObjectHolder со значением аргумента
    runtime::ObjectHolder Execute(runtime::Closure& closure, runtime::Context& context) override;
};
...
public:
    ...
    {}
protected:
    std::unique_ptr<Statement> lhs_;  // protected, чтобы иметь доступ в дочерних классах
    std::unique_ptr<Statement> rhs_;  // protected, чтобы иметь доступ в дочерних классах
};
...
// Составная инструкция (например: программа целиком, тело метода, содержимое ветки if, либо else)
...
public:
    ...
    {
        // Пакет параметров существует только во время компиляции
...
private:
    ...
    // Рекурсивный variadic шаблон для поочередной обработки аргументов
